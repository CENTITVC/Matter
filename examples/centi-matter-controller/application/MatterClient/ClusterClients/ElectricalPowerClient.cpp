#include "ElectricalPowerClient.h"

using namespace chip;
using namespace chip::Controller;
using namespace chip::app::Clusters;

CHIP_ERROR ElectricalPowerClient::SubscribeActivePower(
...
{
    mActivePowerSubscriber.SetTargetEndPoint(endpointId);

    auto onConnectionSuccessCb = [=](Messaging::ExchangeManager & exchangeMgr,
                                    const SessionHandle & sessionHandle) 
    {
        CHIP_ERROR error = CHIP_NO_ERROR;
        mActivePowerSubscriber.SetTargetNode(exchangeMgr, sessionHandle);

        error = mActivePowerSubscriber.SubscribeAttribute(context, reportCb, failureCb, 
